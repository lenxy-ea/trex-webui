...
The separately confirmed daemon `stop-trex` action is the recovery boundary
when exact session authority is no longer available: it receives the same
process-local priority as the hard-stop supervisor, calls `force_trex_kill`,
and only after the daemon confirms that no TRex process remains retires the
durable traffic session and pending traffic WAL as stopped. It never adopts or
mutates traffic in a newer daemon generation.
...
- `POST /api/system/daemon/trex/stop`: require confirmation token `stop-trex` when confirmation is enabled, close the cached STL client under terminating-workload priority, then call daemon `force_trex_kill`; boolean `true` returns `stopped=true`, boolean `false` still proves the daemon is idle and returns `stopped=false` so the UI can log the original dialog message `TRex is not running`, and non-boolean results return `daemon_stop_result_invalid`. After either boolean result, the backend atomically retires any durable traffic session/WAL as stopped using daemon termination evidence; a retirement failure changes the overall response to a blocker without hiding the successful process-stop result.
