...
check_layout() {
  ...
  # ABI v1 remains in the payload as immutable migration input. ABI v2 is the
  # only semantic recovery authority installed by this release.
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-consumer-ack.service" ]] || die "missing recovery ABI v1 consumer acknowledgement unit"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-reconcile.service" ]] || die "missing recovery ABI v1 reconciler unit"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-retry.service" ]] || die "missing recovery ABI v1 retry unit"
  [[ -f "$PROJECT_ROOT/deploy/systemd/nginx-trex-webui-release-reconcile.conf" ]] || die "missing recovery ABI v1 Nginx dependency drop-in"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-consumer-ack-v1-bridge-v2.conf" ]] || die "missing recovery ABI v1 consumer acknowledgement bridge"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-consumer-ack-v2.service" ]] || die "missing recovery ABI v2 consumer acknowledgement unit"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-reconcile-v1-bridge-v2.conf" ]] || die "missing recovery ABI v1 reconciler bridge"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-reconcile-v2.conf" ]] || die "missing recovery ABI v2 consumer dependency drop-in"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-reconcile-v2.service" ]] || die "missing recovery ABI v2 reconciler unit"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-retry-v1-bridge-v2.conf" ]] || die "missing recovery ABI v1 retry bridge"
  [[ -f "$PROJECT_ROOT/deploy/systemd/trex-webui-release-retry-v2.service" ]] || die "missing recovery ABI v2 retry unit"
